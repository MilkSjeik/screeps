...
function getBody(oRoom, sRole) {
  ...
  if (sRole === "miner") {
    ...
    let maxparts = Math.floor(
      (energyAvailable - creepCost) / BODYPART_COST[WORK]
    );
...
function spawnCreep(oRoom, sRole) {
  ...
  sSpawnPoint.spawnCreep(getBody(oRoom, sRole), newName, {
    memory: { role: sRole }
  });
...
function verifyCreeps(oRoom) {
  ...
  if (nMiners <
*/
  const numberOfMinersWanted = _.find(oRoom.memory.creeps, {
    role: "miner"
  }).number;
  const numberOfMinerCreeps = _.filter(
    oRoom.find(FIND_CREEPS),
    (creep) => creep.memory.role == "miner"
  ).length;
  log.msg(
    L.DEBUG,
    "Miners: " + numberOfMinerCreeps + "/" + numberOfMinersWanted
  );
